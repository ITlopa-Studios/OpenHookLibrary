...
struct HookData {
    ...
    void* hook_func; // Указатель на хук функцию
    bool installed;
...
OPENHL_API void OpenHL_Shutdown(void) {
    ...
    for (auto& pair : g_hooks) {
        ...
        if (data.installed) {
            DWORD oldProtect;
            if (ChangeMemoryProtection(data.target, HOOK_JMP_SIZE, PAGE_EXECUTE_READWRITE, &oldProtect)) {
                memcpy(data.target, data.original_bytes.data(), HOOK_JMP_SIZE);
                DWORD tmp;
                ChangeMemoryProtection(data.target, HOOK_JMP_SIZE, oldProtect, &tmp);
...
OPENHL_API OpenHL_Result OpenHL_InstallHook(void* target_address, void* hook_function) {
    ...
    data.hook_func = hook_function; // Сохранение адреса хук функции
    unsigned char* pTarget = (unsigned char*)target_address;
    data.original_bytes.assign(pTarget, pTarget + HOOK_JMP_SIZE);
    DWORD oldProtect;
    if (!ChangeMemoryProtection(target_address, HOOK_JMP_SIZE, PAGE_EXECUTE_READWRITE, &oldProtect)) return OPENHL_ERROR;
    WriteJmp(target_address, hook_function); // Переход на хук функцию
    DWORD tmp;
    ChangeMemoryProtection(target_address, HOOK_JMP_SIZE, oldProtect, &tmp);
    data.installed = true;
